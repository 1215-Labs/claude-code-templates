...
| **tee masks exit code** | Shell pipe `cli ... \| tee log` returns tee's exit code (always 0), not the CLI's. `done.json` always showed `exit_code: 0` even on failure. | Added `set -o pipefail;` prefix to shell commands in both executors. With pipefail, the pipe returns the first non-zero exit code. | Always use `set -o pipefail` when piping CLI output through `tee`. |
| **Stale file collisions** | Same slug produces identical file paths (`/tmp/{tool}-task-{slug}-done.json`). On retries or double-handling, multiple processes clobber each other's output. Orchestrator reads stale `done.json` from a previous run. | Added `Path(stale).unlink(missing_ok=True)` cleanup at executor startup for `done.json` and result files (summary/response). | Executors now self-clean stale output before starting. No manual cleanup needed. |
| **Double-handling** | Orchestrator launched a delegator agent AND manually forked the CLI, both writing to the same files. Seven fork logs observed for what should have been two tasks. | Operational discipline: do not manually intervene when a delegator agent is running. Let it complete or kill it first. | Single-owner principle: either the delegator agent manages the fork, or the orchestrator does — never both. |
